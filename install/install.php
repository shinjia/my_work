...
function do_add_many($add_count) {
    ...
    if($add_count==0) {
        ...
        for($i=1; $i<=$add_count; $i++) {
            // 定義每個資料的範圍及規則
            $workcode = uniqid();
            $workname = chinese_name();
            $intro  = $a_addr[array_rand($a_addr)];
            $descr = @date('Y-m-d', @strtotime('-'.mt_rand(0,650*60).' day'));  // 前六十年內的任一天
            $pub_date   = mt_rand(150, 190);
            $picture   = mt_rand(45, 95);
        ...
        // SQL 語法
        $sqlstr = "INSERT INTO work(workcode, workname, intro, descr, pub_date, picture, remark) VALUES ";
        $sqlstr .= $record_all;
...
function do_create_db() {
    try {
        $pdo = new PDO('mysql:host='.DB_SERVERIP, DB_workname, DB_PASSWORD);
